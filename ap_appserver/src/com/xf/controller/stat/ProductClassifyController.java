...
public class ProductClassifyController {
	...
	@ResponseBody
	private Object updateGroup(@RequestBody String products,
			HttpServletRequest request) {
		...
		try {
			...
			if (products != null) {
				JSONObject objson = JSONObject.fromObject(products);
				if (objson.has("productId")&&objson.getJSONArray("productId").isArray()) {
					List list = objson.getJSONArray("productId");
				...
				if (objson.has("groupId")) {
					s = (String) objson.getString("groupId");
					if (s != null && Tools.isInteger(s))
						prg.setId(Integer.parseInt(s));
				}
				if (objson.has("groupName")) {
					s = (String) objson.get("groupName");
					if (s != null && !s.isEmpty())
						prg.setProductName(s);
				}
				if (objson.has("remark")) {
					s = (String) objson.get("remark");
					if (s != null && !s.isEmpty())
						prg.setRemark(s);
				}
				if (objson.has("trade1")) {
					s = objson.getString("trade1");
					if (s != null && Tools.isInteger(s))
						prg.setTrade1(Integer.parseInt(s));
				}
				if (objson.has("trade2")) {
					s = objson.getString("trade2");
					if (s != null && Tools.isInteger(s))
						prg.setTrade2(Integer.parseInt(s));
				}
				if (objson.has("trade3")) {
					s = objson.getString("trade3");
					if (s != null && Tools.isInteger(s))
						prg.setTrade3(Integer.parseInt(s));
				}
				if (objson.has("trade4")) {
					s = objson.getString("trade4");
					if (s != null && Tools.isInteger(s))
						prg.setTrade4(Integer.parseInt(s));
				}
	...
	private Object findProduct(HttpServletRequest request) {
		...
		try {
			...
			List<CompanyProduct> pList = proService.findProduct(groupid,
					productName, tradeid,year,districtid);
			if (pList.size() > 0) {
