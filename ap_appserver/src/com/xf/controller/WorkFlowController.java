...
//					changeStatusByid3(ids,STATUS_FILL, STATUS_UNCHECK, fillyear, 2);
	...
	public Object getCompanyStatus(HttpServletRequest request) {
		...
		return companyStatusList(num, u, con);
	}
...
//			return result.setStatus(-1, "No login.");
	...
	private Object companyStatusList(int status, User u, Condition con) {
...
//				}
	...
	private void details(List<Integer> companyid, int fillyear) {
		...
		for (DetailsOfindustry c : co) {
			for (DetailsOfindustry comf : com) {
				if (c.getId() == comf.getCompanyId()) {
		...
		if (compro != null) {
			for (DetailsOfindustry proli : compro) {
				if (proli.getProductfId() > 0) {
					List<DetailsOfindustry> commat = detailService.getMaterialBypfid(proli.getProductfId());
		...
		if (comfac != null) {
			for (DetailsOfindustry fac : comfac) {
				if (fac.getFacilityId() > 0) {
					...
					List<DetailsOfindustry> step = detailService.getStepByfaid(fac.getFacilityId());
					if (step != null) {
					...
					List<DetailsOfindustry> devi = detailService.getDeviceByfaid(fac.getFacilityId());
					if (devi != null) {
